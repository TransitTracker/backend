<x-layout>
    <div class="container grid grid-cols-6 px-4 mx-auto mt-8 md:grid-cols-12 gap-y-8 gap-x-12" x-data="{ sector: 0 }">
        <div class="col-span-full">
            ...
            <p>{{ __("Thanks to community effort, exo buses are now identified by fleet numbers in Transit Tracker. Explore exo's directory of buses across their sectors right here.") }} <a href="#about" class="underline">{{ __('Learn more') }}</a></p>
        </div>
        <svg viewBox="0 0 800 619" class="col-span-6 max-w-6xl">
            @foreach($agencies as $agency)
            <a href="{{ route('vin.agency.show', ['sector' => $agency->name_slug]) }}" class="transition-opacity" @mouseover="sector = {{ $agency->id }}" @mouseleave="sector = 0" :class="{'opacity-50': sector === {{ $agency->id }}}">
        ...
        <ul class="col-span-6 space-y-2 block">
            ...
            <li @mouseover="sector = {{ $agency->id }}">
                <a href="{{ route('vin.agency.show', ['sector' => $agency->name_slug]) }}" class="flex items-center gap-x-2 -mx-2 -my-1 md:-my-2 px-2 py-1 md:py-2 rounded" :class="{'bg-white dark:bg-neutral-22': sector === {{ $agency->id }}}">
                    <span class="w-6 h-6 rounded-full flex-shrink-0" style="background-color: {{ $agency->color }};"></span>
                    <div>
                        <p class="text-1xl font-bold xl:text-2xl text-primary-700 dark:text-white font-heading">{{ $agency->short_name }}</p>
        ...
        <div class="col-span-full md:col-span-6">
            ...
            <table class="overflow-x-auto whitespace-nowrap border rounded bg-neutral-96 dark:bg-neutral-10 border-neutralVariant-50 text-neutral-10 dark:text-neutral-90 dark:border-neutralVariant-60  border-collapse block">
                <thead>
                    <tr class="h-14">
            ...
            <table class="overflow-x-auto whitespace-nowrap border rounded bg-neutral-96 dark:bg-neutral-10 border-neutralVariant-50 text-neutral-10 dark:text-neutral-90 dark:border-neutralVariant-60 border-collapse block">
                ...
                <tbody class="border-t divide-y divide-neutralVariant-50 dark:divide-neutralVariant-60">
                    ...
                    <tr class="h-[3.25rem]">
                        ...
                        <td class="px-4 text-left">
                            <a href="{{ route('vin.show', ['vin' => $vehicle->ref]) }}" class="flex items-center justify-between gap-x-2">
                                {{ $vehicle->ref }}
        ...
        <div class="col-span-full">
            ...
            <p class="prose text-white">
                {{ __("Depuis quelques années, les données en temps réel des secteurs d'autobus d'exo identifient leurs bus à l'aide des VIN (identifiant unique du manufacturier). Ce projet vise à identifier tous les bus d'exo avec leur numéro de flotte. Il permet également d'observer les mouvements de véhicules à travers différents secteurs, de les regrouper par opérateurs ainsi que d'obtenir les informations techniques sur chaque bus, extraits à partir du numéro VIN.") }}
                <br>
                {{ __("Ce projet ne serait pas possible sans la contribution de MTL66 à l'identification des bus et à la gestion des suggestions. Merci!") }}
            </p>
        </div>
    </div>
</x-layout>