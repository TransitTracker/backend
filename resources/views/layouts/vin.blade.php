...
<body class="relative min-h-screen font-sans bg-primary-95 dark:bg-neutral-6 text-neutral-10 dark:text-neutral-90">
    <nav
        class="w-full text-xl bg-primary-40 dark:bg-neutral-12 text-white drop-shadow">
        <div class="container flex items-center justify-between px-4 py-4 mx-auto">
            ...
            <a href="{{ route('locale', ['locale' => app()->getLocale() === 'en' ? 'fr' : 'en']) }}"
               class="h-9 px-4 flex items-center justify-center bg-white bg-opacity-0 hover:bg-opacity-10 text-sm font-medium tracking-wider rounded">
                {{ __('FR') }}
            </a>
        </div>
    </nav>
    ...
    <footer class="absolute bottom-0 w-full h-12 mt-20 dark:text-white">
        <div
            class="container flex items-center h-full px-4 mx-auto border-t-4 border-opacity-25 dark:border-opacity-50 border-primary-50">
            <div>
        ...
        <div class="container px-4 mx-auto mt-4 text-sm lg:hidden">
            <div class="bg-primary-10 px-1 py-0.5 text-sm rounded text-white font-medium inline-block">
                {{ __('Cookies') }}</div>
            {{ __('This website uses cookies to show you which suggestions you have made and which suggestions you have approved.') }}
        </div>
    </footer>
