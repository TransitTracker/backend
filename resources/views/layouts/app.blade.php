...
<body class="relative min-h-screen font-sans bg-primary-100 dark:bg-grey-900">
    <nav class="w-full px-4 text-xl text-white bg-primary-500 dark:bg-primary-700 drop-shadow">
        <div class="container flex items-center px-4 py-4 mx-auto md:px-0">
            ...
            <a href="{{ route('locale', ['locale' => app()->getLocale() === 'en' ? 'fr' : 'en']) }}"
                class=" min-w-[64px] h-9 px-4 flex items-center justify-center bg-white bg-opacity-0 hover:bg-opacity-10 text-sm font-medium tracking-wider rounded">{{ __('FR') }}</a>
        </div>
    </nav>
    @if (session('status'))
    ...
    <footer class="absolute bottom-0 w-full h-12 mt-20">
        <div
            class="container flex items-center h-full px-4 mx-auto border-t-4 border-opacity-25 border-primary-500 dark:border-white dark:text-white">
            <div>
                {{ __('A project by') }} <a href="https://felixinx.me/{{ app()->getLocale() === 'en' ? 'en' : '' }}"
                    class="text-[#1f5784] dark:text-white font-bold">@felixinx</a>
            </div>
            <div class="flex-grow"></div>
            <a href="https://github.com/TransitTracker"
                class="transition-colors hover:text-primary-700 dark:text-white dark:hover:text-secondary-500">
