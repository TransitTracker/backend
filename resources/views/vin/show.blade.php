        ...
        <div class="col-span-full font-medium text-[1.375rem] leading-7 text-m3-primary-on-container dark:text-white -mt-6 md:-mt-10 flex items-center gap-x-2" x-data="{ vinInfo: false }">
            ...
            <x-dialog.basic showProp="vinInfo">
                <x-slot:title>{{ __('Aditionnal information based on VIN number') }}</x-slot>
                    ...
                    <a href="https://vpic.nhtsa.dot.gov/api/Home" class=" inline-flex items-center gap-x-1 underline" target="_blank">
                        <span>{{ __('Source') }}</span>
                ...
                <li>
                    <div class="flex items-center py-3 px-4 gap-x-4 @if ($sessionVote === $suggestion->id) bg-green-100 @endif">
                        ...
                        <form action="{{ route('vin.vote', ['suggestion' => $suggestion->id]) }}" method="POST" id="form-vote">
                            ...
                            @csrf
                            <button @if ($sessionVote) disabled @endif class="relative flex items-center justify-center w-9 h-9 before:rounded-full before:absolute before:inset-0 before:bg-black dark:before:bg-white before:opacity-0 hover:before:opacity-5 dark:hover:before:opacity-10 disabled:before:opacity-0 focus:before:opacity-5 disabled:cursor-not-allowed group">
