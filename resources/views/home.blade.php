    ...
    <div class="relative bg-secondary-500 dark:bg-secondary-700 bg-tt-pattern bg-32">
        <div class="py-10 bg-gradient-to-r from-secondary-500 dark:from-secondary-700 to-transparent">
            <div class="container px-4 mx-auto">
                <h1 class="text-4xl font-bold xl:text-6xl text-primary-900 dark:text-white md:w-2/3 lg:w-1/3">{{ __('Making realtime transit data accessible') }}</h1>
            </div>
        </div>
    </div>
    <div class="container grid px-4 mx-auto mt-8 lg:grid-cols-3 gap-y-8 lg:gap-x-12">
        <div class="grid p-4 bg-white rounded-lg shadow-sm dark:bg-grey dark:text-white">
            <div class="flex items-center mb-2">
                ...
                <span class="px-2 py-1 ml-2 mr-2 text-xs font-medium text-white rounded-2xl bg-secondary-700 dark:bg-secondary-900">{{ __('Free!') }}</span>
                <div class="flex-grow"></div>
            ...
            <div class="flex flex-wrap gap-2">
                <a target="_blank" href="https://felixinx.stoplight.io/docs/transit-tracker" class="inline-flex items-center self-end px-4 py-2 text-sm font-medium tracking-wider text-white uppercase rounded shadow bg-primary-500 dark:bg-primary-700 hover:bg-opacity-80 justify-self-start">
                    {{ __('Docs') }}
                    <x-gmdi-open-in-new class="w-5 h-5 ml-2" />
                </a>
                <a target="_blank" href="https://{{ app()->getLocale() === 'en' ? 'status' : 'statut' }}.transittracker.ca" class="inline-flex items-center self-end px-4 py-2 text-sm font-medium tracking-wider text-white uppercase rounded shadow bg-primary-500 dark:bg-primary-700 hover:bg-opacity-80 justify-self-start">
                    {{ __('Status') }}
                    <x-gmdi-open-in-new class="w-5 h-5 ml-2" />
                </a>
            </div>
        ...
        <div class="grid p-4 text-white rounded-lg shadow-sm bg-primary-500 dark:bg-primary-700">
            ...
            <a target="_blank" href="https://www.transittracker.ca/{{ app()->getLocale() === 'en' ? '' : 'fr' }}" class="inline-flex items-center self-end px-4 py-2 text-sm font-medium tracking-wider text-black uppercase bg-white rounded shadow dark:bg-grey dark:text-white hover:bg-opacity-80 justify-self-start">
