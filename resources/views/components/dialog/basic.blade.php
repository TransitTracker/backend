<div {{ $attributes }} tabindex="-1" x-bind:aria-hidden="!{{ $showProp }}">
    <div class="absolute top-0 left-0 h-full w-full flex items-center justify-center pointer-events-none">
        ...
        <div class="bg-neutral-98 dark:bg-neutral-6 p-6 rounded-[1.75rem] text-m3-surface-on-variant dark:text-m3-surface-dark-on-variant min-w-[17.5rem] max-w-[35rem] md:max-h-[35rem] my-12 z-50 pointer-events-auto flex flex-col"
            x-show="{{ $showProp }}" @click.outside="{{ $showProp }} = false"
            x-transition.scale.80.duration.300ms.opacity>
            <h2 class="text-2xl leading-8 text-neutral-10 dark:text-neutral-90 flex-shrink-0">
                {{ $title }}</h2>
            <p class="mt-4 leading-5 text-sm flex-shrink-0">{{ $slot }}</p>
            @if ($body ?? null)
                <hr class="my-3 border-t border-neutralVariant-90 dark:border-neutralVariant-30 flex-shrink-0">
