...
<div class="container grid grid-cols-1 px-4 mx-auto mt-8 md:grid-cols-3 gap-y-8 gap-x-12">
    ...
    <div class="col-span-full font-medium text-[1.375rem] leading-7 text-m3-primary-on-container dark:text-white -mt-6 md:-mt-10 flex items-center gap-x-2" x-data="{ vinInfo: false }">
        <h2>{{ $vinInfo->getManufacturer() }} {{ $vinInfo->getModel() }} {{ $vinInfo->getYear() }}</h2>
        ...
        <x-dialog.basic showProp="vinInfo">
            <x-slot:title>{{ __('Aditionnal information based on VIN number') }}</x-slot>
                {{ __('This information is generated automatically from data taken from the CPTDB wiki.') }}
                @if($vinInfo->getSource())
                <a href="{{ $vinInfo->getSource() }}" class=" inline-flex items-center gap-x-1 underline" target="_blank">
                    <span>{{ __('Source') }}</span>
                ...
                <x-slot:body>
                    <dl class="text-sm">
                        ...
                        @if($vinInfo->getPropulsion())
                        <dt class="mt-2 text-xs">{{ __('Propulsion') }}</dt>
                        <dd class="mt-0.5">
                            {{ $vinInfo->getPropulsion() }}
                        </dd>
                        @endif
                        @if($vinInfo->getLength())
                        ...
                        @endif
                        @if($vinInfo->getNote())
                        <p class="mt-4 italic">{{ $vinInfo->getNote() }}</p>
                        @endif
                    </dl>
                    </x-slot>
                    <x-slot:actions>
                        <div class="flex">
                            <x-button.text href="#" :hasIcon="false" @click="vinInfo = false" class="ml-auto">Close</x-icon-button.text>
                        </div>
                        </x-slot>
        </x-dialog.basic>
    </div>
    ...
    <div class="md:col-span-2">
        <ul class="space-y-2">
            ...
            <li class="flex-shrink-0 py-2 px-4 mb-2 space-y-2 rounded-2xl bg-m3-surface-variant text-m3-surface-on-variant dark:bg-m3-surface-dark-variant dark:text-m3-surface-dark-on-variant" x-data="{ open: false }">
                <div class="flex items-center gap-x-4">
                    ...
                    <div class="items-center rounded px-2 py-1" title="{{ $tag->description }}" style="background-color: {{ $tag->color }};color: {{ $tag->text_color }}">{{ $tag->label }}</div>
                    @endforeach
                    <div class="grow"></div>
                    <x-button.text :has-icon="true" href="{{ route('vin.agency.show', ['agency' => $vehicle->agency->slug]) }}" class="text-m3-surface-on-variant dark:text-m3-surface-dark-on-variant">
                ...
                <div class="flex items-center gap-x-1 text-xs">
                    ...
                    @else
                    <button class="flex items-center gap-x-1" x-on:click="open = !open">{{ __('Show latest info') }}
                        <x-gmdi-keyboard-arrow-down class="w-4 h-4 text-current transition-transform" x-bind:class="open ? 'rotate-180' : ''" />
                    </button>
                    @endif
                </div>
                <div class="-mx-4 mt-2" x-show="open" x-collapse>
                    ...
                    <dl class="border-t border-m3-surface-on-variant/25 dark:border-m3-surface-dark-on-variant/25 px-4 py-2">
                        ...
                        <dd>
                            <a href="https://www.openstreetmap.org/?mlat={{ $vehicle->lat }}&mlon={{ $vehicle->lon }}#map=16/{{ $vehicle->lat }}/{{ $vehicle->lon }}&layers=T" target="_blank" class="flex gap-x-1 items-center underline">
                                <x-gmdi-pin-drop class="w-4 h-4 text-current" /> {{ $vehicle->lat }},
                                {{ $vehicle->lon }}
                                @if ($vehicle->bearing)
                                <x-gmdi-north class="w-4 h-4 text-current" style="transform:rotate({{ $vehicle->bearing }}deg)" />
                                @endif
                            </a>
                            @if ($vehicle->speed)
                            <div class="flex gap-x-1 items-center">
        ...
        <form action="" method="POST" class="p-4 mt-6 border rounded-2xl text-m3-surface-on dark:bg-m3-surface-dark dark:text-m3-surface-dark-on border-m3-background-outline dark:border-m3-background-dark-outline" id="form-suggest">
            @csrf
            @env('local')
            <input type="hidden" value="good" id="g-recaptcha-response" name="g-recaptcha-response" />
            @endenv
            ...
            <div class="mb-4 text-xs font-medium text-red-500">
                {{ __('The reCAPTCHA validation has failed. Please try again.') }}
            </div>
            @enderror

            <div class="flex">
                <button data-sitekey="{{ config('transittracker.recaptcha.site') }}" data-callback='onSuggestSubmit' class="relative flex items-center h-10 px-6 text-sm font-medium transition-colors rounded-full bg-m3-primary text-m3-primary-on dark:bg-m3-primary-dark dark:text-m3-primary-dark-on hover:bg-opacity-85 focus:bg-opacity-75 g-recaptcha">
                    {{ __('Send') }}
            ...
            <li>
                <div class="flex items-center py-3 px-4 gap-x-4 @if ($sessionVote === $suggestion->id) bg-green-100 @endif">
                    ...
                    <div x-data="{ showModal: false }">
                        ...
                        <div class="absolute inset-0 z-50 flex items-center justify-center w-full h-full bg-black bg-opacity-50" x-show="showModal">
                            <div class="bg-white dark:bg-m3-surface-dark dark:text-m3-surface-dark-on rounded shadow-xl w-[280px]" x-on:click.away="showModal = false">
                                ...
                                <div class="pl-6 pr-2">
                                    @foreach ($vehicles as $vehicle)
                                    <form action="{{ route('vin.approve', ['suggestion' => $suggestion->id, 'agency' => $vehicle->agency->slug]) }}" method="POST">
                                        @csrf
                    ...
                    <form action="{{ route('vin.reject', ['suggestion' => $suggestion->id]) }}" method="POST">
                        @csrf
                        <button type="submit" class="relative flex items-center justify-center w-9 h-9 before:rounded-full before:absolute before:inset-0 before:bg-black before:opacity-0 hover:before:opacity-5 focus:before:opacity-5 disabled:cursor-not-allowed group">
